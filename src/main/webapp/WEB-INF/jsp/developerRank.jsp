...
<div id="bodyy" class="packages-list-container" style="margin-top: 80px">
    <div class="container">
        <div style="margin-top: 30px"><p style="text-align: center;font-size: 20px">开发者排行榜</p></div>
...
<script>
    ...
    function init(page) {
        $.ajax({
            url:'<%=base%>getDeveloperRankData?page=' + page,
            dataType:"json",
            type:"GET",
            async:true,
...
//                $('#total').html(object.total);
                ...
                for (var i in object.list) {
                    html += "<tr><td>"+(parseInt(i)+1+(parseInt(page)-1)*10)+"</td>" +
                        "<td><a href='https://github.com/"+isEmpty(object.list[i].login)+"' target='_blank'>"+isEmpty(object.list[i].login)+"</a></td>" +
                        "<td>"+isEmpty(object.list[i].name)+"</td>" +
                        "<td>"+isEmpty(object.list[i].company)+"</td>" +
                        "<td>"+isEmpty(object.list[i].location)+"</td>" +
                        "<td>"+isEmpty(object.list[i].publicRepos)+"</td>"+
                        "<td>"+isEmpty(object.list[i].followers)+"</td>"+
                        "<td>"+timestampToTime(object.list[i].createdAt)+"</td></tr>";
                }
                $('#body').html(html);
            }
        });
    ...
    function timestampToTime(timestamp) {
        var date = new Date(timestamp);//如果date为10位需要乘1000
